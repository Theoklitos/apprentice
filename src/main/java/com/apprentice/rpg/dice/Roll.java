...
public final class Roll {
	 ...
	 */
	public void addDice(final int diceToAdd) {
	 ...
	 * @throws ApprenticeEx
	 */
	public void addDice(final int diceToAdd, final int position) {
		if (position == 0) {
	 ...
	 */
	public void addRoll(final Roll toAdd) {
		// add the modifier
	 ...
	 * 2D6.
	 */
	public List<String> getMergedDicesAsStringList(final boolean showPlusOperators) {
	 ...
	 * exist.
	 */
	public int removeDice(final int diceToRemove) {
	 ...
	 */
	public int removeOneOccurenceOfDice(final int diceToDecrease) {
		return dice.removeOnce(diceToDecrease);
	}

	/**
	 * Rolls itself and returns result. Uses java.random.
	 */
	public int roll() {
		final Random random = new Random();
		int result = 0;
		for (int i = 0; i < dice.size(); i++) {
			final int currentDice = dice.getOccurencesOf(i);
			for (int z = 0; z < dice.getOccurencesOf(currentDice); z++) {
				result += (random.nextInt(currentDice));
				if (currentDice >= 0) {
					result++;
				} else {
					result--;
		...
		final int total = result + modifier;
